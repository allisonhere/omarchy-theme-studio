...
pub fn process_key(app: &mut App, key: crossterm::event::KeyEvent) -> bool {
    ...
    match app.input_mode {
        InputMode::Preview => match key.code {
            ...
            KeyCode::Char(c @ '1'..='6') => {
                app.select_group_index(c as usize - '1' as usize);
                app.message = None;
            }
            KeyCode::Char('/') => app.open_field_search(),
        ...
        InputMode::ColorPicker => {
            if app.color_editor.is_editing_text() {
                ...
                match key.code {
                    ...
                    KeyCode::BackTab => app.color_editor.focus_next(true),
                    KeyCode::Tab => app.color_editor.focus_next(false),
                    KeyCode::Char('m') => app.color_editor.toggle_mode(),
                    KeyCode::Char('#') => app.color_editor.start_hex_input(),
                    KeyCode::Char('y') => app.yank_color(),
                    KeyCode::Up => {
        ...
        InputMode::ThemeNameInput => match key.code {
            ...
            KeyCode::Enter => app.save_theme_as_input_name(),
            KeyCode::Backspace => app.pop_theme_name_char(),
            KeyCode::Char(c) => app.push_theme_name_char(c),
            _ => {}
        },
        InputMode::ApplyConfirm => match key.code {
            KeyCode::Char('y') | KeyCode::Char('Y') | KeyCode::Enter => app.confirm_apply(),
            KeyCode::Char('n') | KeyCode::Char('N') | KeyCode::Esc => app.cancel_apply(),
            _ => {}
        },
        InputMode::FieldSearch => match key.code {
            KeyCode::Esc => app.cancel_field_search(),
            KeyCode::Enter => app.commit_field_search(),
            KeyCode::Up => app.move_field_search(-1),
            KeyCode::Down => app.move_field_search(1),
            KeyCode::Backspace => app.pop_field_search_char(),
            KeyCode::Char(c) => app.push_field_search_char(c),
            _ => {}
        },
        InputMode::ThemeLoad => {
            if app.search_focused {
                ...
                match key.code {
                    ...
                    KeyCode::Enter => app.load_selected_theme(),
                    KeyCode::Char('d') => app.set_theme_filter(crate::ui::state::ThemeFilter::Builtin),
                    KeyCode::Char('s') => app.set_theme_filter(crate::ui::state::ThemeFilter::Saved),
                    KeyCode::Char('r') => app.begin_rename_selected_theme(),
                    KeyCode::Char('x') => app.begin_delete_selected_theme(),
                    KeyCode::Up => app.move_theme_selection_up(),
                    KeyCode::Down => app.move_theme_selection_down(),
                    KeyCode::Backspace => {
                        if !app.theme_search_query.is_empty() {
                            app.theme_search_query.pop();
        ...
        InputMode::ThemeLoadRename => match key.code {
            ...
            KeyCode::Enter => app.commit_rename_theme(),
            KeyCode::Backspace => app.pop_theme_name_char(),
            KeyCode::Char(c) => app.push_theme_name_char(c),
            _ => {}
        },
        InputMode::ThemeLoadDeleteConfirm => match key.code {
            KeyCode::Char('y') => app.confirm_delete_theme(),
        ...
        InputMode::UpdateRestartConfirm => match key.code {
            ...
            KeyCode::Char('l') | KeyCode::Char('n') | KeyCode::Esc => app.defer_restart(),
            _ => {}
        },
    }
    false
        ...
        if ct_event::poll(Duration::from_millis(100))? {
            match ct_event::read()? {
                ct_event::Event::Key(key) => {
                    ...
                    // A command that shelled out (e.g. apply) may have drawn over
                    // the alternate screen — force a full repaint.
                    if app.force_redraw {
