...
const REPO: &str = "allisonhere/omarchy-theme-studio";
...
fn release_asset_name() -> Option<&'static str> {
    match (std::env::consts::OS, std::env::consts::ARCH) {
        ("linux", "x86_64") => Some("omarchy-theme-studio-linux-x86_64"),
        _ => None,
    }
}
...
pub fn check_version() -> Result<Option<String>, String> {
    ...
    let resp: serde_json::Value = http_agent()
        .get(&url)
        .set("User-Agent", "omarchy-theme-studio")
        .call()
...
mod tests {
    ...
    fn release_asset_matches_supported_targets() {
        match (std::env::consts::OS, std::env::consts::ARCH) {
            ("linux", "x86_64") => {
                assert_eq!(release_asset_name(), Some("omarchy-theme-studio-linux-x86_64"));
            }
